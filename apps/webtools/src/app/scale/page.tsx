...
export default function ScalePage() {
  ...
  useEffect(() => {
    const storedFiles = retrieveFiles('scale');
  ...
  const handleProcessed = (scaledFileId: string) => {
    if (!uploadedFile) return;
    setProcessedFile({ ...uploadedFile, id: scaledFileId, status: 'processed' });
    setView('preview');
  }
  ...
  const handleDelete = async () => {
    ...
    removeFile('scale', uploadedFile.id);
    if (processedFile) removeFile('scale', processedFile.id);
