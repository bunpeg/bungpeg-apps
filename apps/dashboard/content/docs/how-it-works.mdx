...
The `/meta/{file_id}` will not store it in DB though. You can also call a `/probe/{file_id}` endpoint to get a lot more information from the file, it is a lot slower too.
